            ...
            <v-col cols="8">
              ...
              >
                ...
                <template v-slot:[`item.delete`]="{ item }">
                  <v-btn
                    v-if="showDeletePatternBtn"
                    @click.stop="onClickDeletePattern(item)"
                    color="#00ffd0"
                    elevation="3"
                    outlined
                    fab
  ...
  methods: {
    ...
    async onTouchSave() {
      ...
      if (this.pageContents === 1) {
        ...
        let selectworkTypeId = JSON.parse(
          JSON.stringify(this.ownCompanyData.workTypeIdList)
        );
    ...
    createOwnCompanyData(response) {
      ...
      for (var i = 0; i < ownCompanyResponse.workTypeIdList.length; i++) {
        var selectWorkType = {};
        selectWorkType.workTypeName = this.getWorkTypeName(
          ownCompanyResponse.workTypeIdList[i],
          workTypeResponse
        );
      ...
      ownCompanyData.normalWorkStartHours = ownCompanyResponse.normalWorkStartTime.split(
        ":"
      )[0];
      ownCompanyData.normalWorkStartMinutes = ownCompanyResponse.normalWorkStartTime.split(
        ":"
      )[1];
      ownCompanyData.normalWorkFinishHours = ownCompanyResponse.normalWorkFinishTime.split(
        ":"
      )[0];
      ownCompanyData.normalWorkFinishMinutes = ownCompanyResponse.normalWorkFinishTime.split(
        ":"
      )[1];
      ownCompanyData.exceptionWorkStartHours = ownCompanyResponse.exceptionWorkStartTime.split(
        ":"
      )[0];
      ownCompanyData.exceptionWorkStartMinutes = ownCompanyResponse.exceptionWorkStartTime.split(
        ":"
      )[1];
      ownCompanyData.exceptionWorkFinishHours = ownCompanyResponse.exceptionWorkFinishTime.split(
        ":"
      )[0];
      ownCompanyData.exceptionWorkFinishMinutes = ownCompanyResponse.exceptionWorkFinishTime.split(
        ":"
      )[1];
