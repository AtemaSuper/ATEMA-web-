    ...
    <v-container>
      <v-sheet color="white" rounded outlined>
        ...
        <v-dialog v-model="employeeDialog" persistent max-width="600px">
          <v-card>
              ...
              <v-container>
                ...
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <div v-if="!employeeEditFlag" class="dialog-label">
                      {{
                        employeeEditItem.selectPost
                          ? employeeEditItem.selectPost["postName"]
                          : ""
                      }}
                    </div>
                    <div v-if="employeeEditFlag">
                      ...
                      >
                        ...
                        >
                          <v-btn
                            ...
                            rounded
                            @click="
                              $refs.birthdayMenu.save(employeeEditItem.birthday)
                            "
                          >
                            OK
                          </v-btn>
                ...
                <v-row>
                  ...
                  <div v-if="!employeeEditFlag" class="dialog-to-label">
                    {{
                      employeeEditItem.telNumber1 +
                        "-" +
                        employeeEditItem.telNumber2 +
                        "-" +
                        employeeEditItem.telNumber3
                    }}
                  </div>
                ...
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <ul>
                      <li
                        v-for="(item, index) in employeeEditItem.license"
                        :key="index"
                      >
                        <div v-if="!employeeEditFlag" class="dialog-label">
                          {{ item }}
                    ...
                    <v-btn
                      v-if="
                        employeeEditFlag && employeeEditItem.license.length <= 9
                      "
                      color="#ff6669"
                      class="white--text"
                      fab
                    ...
                    <v-btn
                      v-if="
                        employeeEditFlag && employeeEditItem.license.length != 1
                      "
                      color="#00ffd0"
                      elevation="3"
                      outlined
            ...
            <v-card-actions>
              ...
              <v-btn
                @click="onClickEmployeeCancelBtn()"
                class="#f5f5f5"
                rounded
        ...
        <v-dialog v-model="employeeDeleteConfirmDialog" max-width="500px">
          <v-card>
            <v-card-title align="center">この社員を削除しますか?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
        ...
        <v-dialog v-model="postDeleteConfirmDialog" max-width="500px">
          <v-card>
            <v-card-title align="center">この役職を削除しますか?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="primary" @click="onClickDeletePost">
                OK
              </v-btn>
              <v-btn color="white" @click="postDeleteConfirmDialog = false">
                キャンセル
              </v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
                ...
                <v-row>
                  <v-col cols="2"
                    ><v-select
                      ...
                      v-model="postEditItem.attendanceManageAuth"
                      item-text="label"
                      item-value="value"
                      label="権限を選択"
                      solo
                  ...
                  <v-col cols="2"
                    ><v-select
                      :items="postPermissionList"
                      v-model="postEditItem.ownWorkerManageAuth"
                      item-text="label"
                      item-value="value"
                      label="権限を選択"
                      solo
  ...
  computed: {
    ...
    postListHeader() {
      return [
        ...
        {
          text: "出退勤管理",
          value: "attendanceManageAuthName",
          align: "center",
          width: "14%"
        },
        {
          text: "自社員管理",
          value: "ownWorkerManageAuthName",
          align: "center",
          width: "14%"
        },
        {
  ...
  methods: {
    ...
    async onClickEmployeeEditBtn() {
      if (this.employeeEditFlag) {
        ...
        if (this.employeeEditItem.selectPost != null) {
          let selectPost = JSON.parse(
            JSON.stringify(this.employeeEditItem.selectPost)
          );
    ...
    async onClickPostEditBtn() {
      const param = {
        ...
        ownWorkerManageAuth: this.postEditItem.ownWorkerManageAuth,
...
function createEmployeeList(response) {
  ...
  for (var i = 0; i < employeeResponse.length; i++) {
    ...
    employee.selectPost = {
      ...
      postName: getPostName(employeeResponse[i].postId, postResponse)
    };
...
function createPostList(response) {
  ...
  for (var i = 0; i < postResponse.length; i++) {
    ...
    post.attendanceManageAuth = postResponse[i].attendanceManageAuth;
    post.ownWorkerManageAuth = postResponse[i].ownWorkerManageAuth;
    ...
    post.attendanceManageAuthName = getAutnName(
      postResponse[i].attendanceManageAuth
    );
    post.ownWorkerManageAuthName = getAutnName(
      postResponse[i].ownWorkerManageAuth
    );
    post.subCompanyManageAuthName = getAutnName(
      postResponse[i].subCompanyManageAuth
    );
    post.ownCompanyManageAuthName = getAutnName(
      postResponse[i].ownCompanyManageAuth
    );
